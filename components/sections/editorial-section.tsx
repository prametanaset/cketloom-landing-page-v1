...
export function EditorialSection() {
  ...
  }, [updateParallax, isMounted]);

  // Parallax effect: video moves up as you scroll down
  const parallaxY = isMounted ? (scrollProgress - 0.5) * 30 : 0; // -15px to +15px range

  return (
    <section className="bg-background" suppressHydrationWarning>
      {/* Newsletter Banner */}
      ...
      <div className="grid grid-cols-2 border-t border-border md:grid-cols-4">
        {specs.map((spec) => (
          <div
            key={spec.label}
            className="border-b border-r border-border p-3 sm:p-4 md:p-8 text-center last:border-r-0 md:border-b-0"
          >
            <p className="mb-2 text-xs uppercase tracking-widest text-muted-foreground">
              {spec.label}
            </p>
            <p className="font-medium text-foreground text-2xl sm:text-3xl md:text-5xl">
              {spec.value}
            </p>
          </div>
        ))}
